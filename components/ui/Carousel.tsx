...
export const SimpleCaroussel = ({ data }: any) => {
        ...
        <View style={styles.container}>
            <ScrollView ref={scrollViewRef} horizontal pagingEnabled showsHorizontalScrollIndicator={false}>
                {data.map((item: any, index: number) => (
                    <Animated.View  key={index} style={[styles.slide, { width }, {opacity: index === 0 ? fadeAnim : 1}]}>
                        <CarouselCard info={{ barcode: item }}></CarouselCard>
                    </Animated.View>
                ))}
            </ScrollView>
        </View>
